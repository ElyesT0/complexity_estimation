...
def prepare_dir(root_path):
    ...
    directories = [
        ...
        f'{figure_path}/interclick/differential',
...
#--------------------------------------------------
def plot_individual_interclick(data,path,expression=True,x_axis_num=False,z_score=False):
...
#--------------------------------------------------
def plot_mean_dl(data,path,print_values=True,save=True):
    # Participants IDs
    ...
    if print_values:
        for i in range(len(all_sem)):
            print(f'[{seq_name_list[i]}] mean DL distance: {round(np.nanmean(distDL_perParticipant[i]),2)}')
            print(f'[{seq_name_list[i]}] SEM: {round(all_sem[i],2)}\n')
...
# Plotting
def plot_mean_error_rates(data,path,print_values=True,save=True,seq_expression=False):
    # Holder Objects
    all_error_rates_seq=[]
    ...
    for i in range(len(all_error_rates_seq)):
        ...
        if print_values:
            print(f'[{seq_name_list[i]}] error rate: {mean_error_holder}')
            print(f'[{seq_name_list[i]}] SEM: {sem_holder}\n')
    ...
    for i in range(len(seq_name_list)):
        ax.errorbar(mean_per_participant_error_rates[i], i, fmt=figure_format_points[i],
                    xerr=sem_per_participant_error_rates[i], capsize=5, color=color_structure_control[i], markersize=8)

    ax.set_yticks(np.arange(len(seq_name_list)))
    
    if seq_expression:
        ax.set_yticklabels(list_seq_expression, fontsize=14)
    else:
        ax.set_yticklabels(seq_name_list, fontsize=14)
        

    ax.invert_yaxis()
    ax.set_xlabel("Mean Error Rate per sequence", fontsize=title_size, labelpad=padding_size)


    if save:
        if seq_expression:
            plt.savefig(f'{path}/mean_errorRates_allSequences_expression.jpg', 
                        bbox_inches='tight', dpi=800)
        else:
...
def chunking_base_interclick(data, path, break_duration=30):
    ...
    for index_name in range(len(seq_name_list)):
        ...
        plt.savefig(f'{path}/interclick/differentials/differential_interclick_{seq_name_list[index_name]}.png', bbox_inches='tight', dpi=800)
        plt.show()
