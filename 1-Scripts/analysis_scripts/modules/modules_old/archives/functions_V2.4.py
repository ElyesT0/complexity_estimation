...
#--------------------------------------------------
def plot_mean_dl(data,path,print_values=True,seq_expression=False,save=True):
    # Participants IDs
    ...
    if seq_expression:
        ...
        for tick, (label, color) in zip(sec_yticklabels, yticklabels):
            tick.set_color(color)
    #_-----
...
# Plotting
def plot_mean_error_rates(data,path,print_values=True,save=True,seq_expression=False,sequences=seq_name_list,colors_figure=plot_colors):
    # Holder Objects
    all_error_rates_seq=[]
...
def t_test_on_OLS(betas):
    print("""Conducting a t-test on aggregate betas. Betas come from a Ordinary Least Square regression of the dependant variable (LoT Complexity)
            on the independant variable (Damerau-Levenshtein distance). We then ran a t-test to observe if our betas distribution is 
