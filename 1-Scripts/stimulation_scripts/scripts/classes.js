...
class ExperimentCl {
  constructor(sequence_training, sequences_structure) {
    this.participant_id = makeId();
    this.sequences_structure = sequences_structure; // Record original sequence temporal structure
    this.sequences_shown = randomize_points(shuffle(sequences_structure)); // Record actual sequence shown to participant (after randomizing spatial positions and order of presentation)
    this.seq = [...sequence_training, ...this.sequences_shown];
    this.element_selectors = {};
    this.counter = 0;
    this.presentation_time = true;
    this.start_time = Date.now();
    this.click_timings = []; // Timings of participant's answers
    this.response = []; //responses of the participant
    this.screen_width = window.screen.width;
    this.screen_height = window.screen.height;
    this.set_delay = set_delay;
    this.break_time = break_time;
    this.SOA = SOA;
    this.stimulus_duration = blink;
    this.state = 'training';
    this.survey = sessionStorage.getItem('surveyChoices'); //retrieve the participant's survey from last page
    this.last_click = Date.now(); // Useful to control for clicks that are too quick (example: double taps)
  }

  /**
   * Draws the figure on the screen by creating and styling various UI elements
   * including circles, buttons, and text. The method calculates screen dimensions
   * and browser bar heights to ensure proper positioning and layout of elements.
   */
  draw() {
    /**
     * Calculates the heights of the browser's URL bar and bottom bar.
     *
     * @returns {Object} An object containing the URL bar height and bottom bar height.
     */
    ...
    // Create and style the fixation cross
    ...
    // Create circles and arrange them in a hexagonal shape
    var circleElements = [];
    for (let i = 0; i < 6; i++) {
    ...
    // Create rating buttons and other UI elements
    var btn_rating = [];
    var container_rating = document.createElement('div');
    container_rating.classList.add('container-rating', 'hidden', 'no--zoom');
    containerFigureElement.appendChild(container_rating);
    container_rating.style.left = centerX_containerFigure / 5 + 'px';
    container_rating.style.width = `${screen_size_width / 1.25}px`;

    for (let i = 1; i < 8; i++) {
      var div = document.createElement('div');
      div.id = `rating-${i}`;
      div.classList.add('btn-rating', 'no--zoom');
      div.textContent = `${i}`;
      container_rating.appendChild(div);
      btn_rating.push(div);
    }

    // Create and position various buttons: Training, OK, Next, and score texts
    let btn_training = document.createElement('div');
    btn_training.textContent = 'TRAINING';
    btn_training.classList.add('btn--training', 'no--zoom', 'hidden');
    ...
    btn_next.classList.add('btn--next', 'no--zoom', 'hidden');
    ...
    // Manage the progression bar size and location
    ...
    /* Add created elements to this.element_selectors for future use in animations and event listeners */
    this.element_selectors.circles = circleElements;
    this.element_selectors.fixation = fixationElement;
    this.element_selectors.rating = btn_rating;
    this.element_selectors.container_rating = container_rating;
  ...
  init() {
    ...
    for (let i = 0; i < 7; i++) {
      this.element_selectors.rating[i].addEventListener(keyEvent, () => {
...
----------------------------------------------------------------------------------------------------------*/
  ...
  hideFigure() {
    ...
    this.element_selectors.container_rating.classList.add('hidden');
    this.element_selectors.progression_bar.classList.add('hidden');
  ...
  display_score_update(rating) {
    let feedbackTXT = rating;
...
----------------------------------------------------------------------------------------------------------*/
  ...
  reproduction() {
    ...
    this.element_selectors.container_rating.classList.remove('hidden');
  }
